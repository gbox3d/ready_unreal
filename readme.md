...
- [5. Claude Code 연결](#5-claude-code-연결)
...
> 클라이언트 등록(부록 A)은 전역이라 1회면 끝나지만, **에디터 측(서버)** 은 프로젝트별
...
2. Claude 쪽은 **손댈 것 없음** — 전역(user) 등록이 이미 되어 있으므로(부록 A),
   Claude 세션에서 `/mcp` → **리커넥트**만 하면 `unreal-mcp ✓ connected`.
